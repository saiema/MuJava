...
public class MultiMutations {
	...
	SinglyLinkedListNode, next).has(n); n.value==value_param)
	   ...
	   public boolean contains( /*@nullable@*/ Object value_param )
	   {
	       ...
	       boolean result;
	       current.next = header;
	       boolean res = current.next == null && current.hashCode() + header.hashCode() > 0; //mutGenLimit 10
	       Object test = current; 
	       result = false;
	       while (result == false && current != null) {
	           boolean equalVal;
	           if (value_param == null && current.value == null) {
	               equalVal = true; //mutGenLimit 2
	           } else {
	               if (value_param != null) {
	           ...
	           current = current.next; //mutGenLimit 3
	       }
	       return result;
	   }
	...
	public static void main(String[] args) {
		MultiMutations list = new MultiMutations();
