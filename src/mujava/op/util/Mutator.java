...
public class Mutator extends mujava.openjava.extension.VariableBinder {
	...
	// --------------
	public OJClass getType(Expression p) throws ParseTreeException // modified
	// (10/09/14)
	// [simon]
	// {protected
	// ->
	// public}
	{
		...
		try {
			...
			// do not throw the exception; otherwise, the program stops
			// generating mutants
	...
	public Map<OJClass, List<Variable>> getReachableVariables(
			ParseTreeObject exp) throws ParseTreeException {
		...
		while (current != null && !(current instanceof MethodDeclaration)) {
			if (current instanceof VariableDeclarator) {
				addVar(variables, allVars, new Variable(
						((VariableDeclarator) current).getVariable()));
			} else if (current instanceof VariableDeclaration) {
				addVar(variables, allVars, new Variable(
						((VariableDeclaration) current).getVariable()));
		...
		if (current instanceof MethodDeclaration) {
			ParameterList params = ((MethodDeclaration) current)
					.getParameters();
	...
	private void addVar(Map<OJClass, List<Variable>> map,
			List<Variable> allVars, Variable var) throws ParseTreeException {
		if (!find(allVars, var)) {
	...
	public OJMethod[] getInheritedMethods(OJClass clazz, boolean filterFinal) {
		...
		while (base != null) {
			OJMethod[] declaredMethods = filterPrivate(
					base.getDeclaredMethods(), filterFinal);
	...
	public OJField[] getInheritedFields(OJClass clazz, boolean filterFinal) {
		...
		while (base != null) {
			OJField[] declaredFields = filterPrivate(base.getDeclaredFields(),
					filterFinal);
	...
	private OJField[] filterPrivate(OJField[] fields, boolean filterFinal) {
		...
		for (OJField f : fields) {
			if ((filterFinal && f.getModifiers().isFinal())
					|| f.getModifiers().isPrivate()) {
				continue;
			}
			filtered.add(f);
		}
	...
	public List<Object> fieldsMethodsAndVars(ParseTreeObject limit, OJClass t,
			boolean ignoreVars, boolean ignoreVoidMethods,
			boolean ignoreMethodsWithParams) throws ParseTreeException {
	 ...
	 * @throws ParseTreeException
	 */
	public void bindLocalVariables(StatementList statList)
			throws ParseTreeException {
	...
	public OJMethod getMethod(String name, OJClass from,
			ExpressionList complyWith) throws ParseTreeException {
	...
	public static ParseTreeObject getStatement(ParseTreeObject node) {
		ParseTreeObject current = node;// (ParseTreeObject)
